	...
	const renderStep = () => {
		switch (step) {
				...
				return (
					<div className="space-y-6">
						...
						<div className="bg-white p-4 rounded-xl border-2 border-primary-200 shadow-sm">
							...
							/>
							<p className="text-xs text-gray-500 mt-2">We use your name to personalize your journey and celebrations.</p>
						</div>

						<div className="bg-primary-50 p-4 rounded-lg">
							<p className="text-sm text-primary-800">
								✨ This app will help you reduce consumption gradually, send you notifications when you're allowed to smoke, and
			...
			case 1:
				return (
					<div className="space-y-6">
						<h2 className="text-2xl font-bold text-gray-800 text-center">Current Consumption</h2>
						<p className="text-gray-600 text-center">How much do you currently smoke or vape per week?</p>
						...
						<div className="space-y-4">
							...
							>
								...
								<input
									...
									className={`input-field ${validationErrors.cigarettesPerWeek ? "border-red-300" : ""}`}
									placeholder="e.g., 70 (about 10 per day)"
								/>
								{validationErrors.cigarettesPerWeek && (
									<p className="text-sm text-red-600 mt-2">{validationErrors.cigarettesPerWeek}</p>
								)}
							</div>
							...
							>
								...
								<input
									...
									className={`input-field ${validationErrors.vapesPerWeek ? "border-red-300" : ""}`}
									placeholder="e.g., 140 (about 20 per day)"
								/>
								{validationErrors.vapesPerWeek && <p className="text-sm text-red-600 mt-2">{validationErrors.vapesPerWeek}</p>}
							</div>
						</div>
			...
			case 2:
				return (
					<div className="space-y-6">
						<h2 className="text-2xl font-bold text-gray-800 text-center">Choose Your Pace</h2>
						<p className="text-gray-600 text-center">How quickly would you like to reduce?</p>
							...
							<div className="flex items-start gap-3">
								...
								<div className="flex-1">
									...
									<p className="text-xs text-purple-700">
										Your plan adjusts based on your actual behavior. Includes minimum wait rules to build better habits and
						...
						<div className="space-y-3">
							<button
								...
								className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
									formData.planSpeed === "slow" ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white"
								}`}
							>
							...
							<button
								...
								className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
									formData.planSpeed === "medium" ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white"
								}`}
							>
								<div className="flex items-center justify-between">
									<div>
							...
							<button
								...
								className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
									formData.planSpeed === "quick" ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white"
								}`}
							>
								<div className="flex items-center justify-between">
									<div>
			...
			case 3:
				return (
					<div className="space-y-6">
						<h2 className="text-2xl font-bold text-gray-800 text-center">Cost Information</h2>
						<p className="text-gray-600 text-center">Help us calculate your savings</p>
							...
							<div className="grid grid-cols-2 gap-3">
								<button
									...
									className={`p-4 rounded-xl border-2 transition-all text-center ${
										formData.currency === "USD" ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white"
									}`}
								>
								...
								<button
									...
									className={`p-4 rounded-xl border-2 transition-all text-center ${
										formData.currency === "EUR" ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white"
									}`}
								>
									<div className="text-2xl mb-1">€</div>
						...
						{parseInt(formData.cigarettesPerWeek) > 0 && (
							<div className="space-y-2">
								...
								<input
									...
									className={`input-field text-lg ${validationErrors.cigarettePrice ? "border-red-300" : ""}`}
									placeholder="8.00"
								/>
								{validationErrors.cigarettePrice && <p className="text-sm text-red-600">{validationErrors.cigarettePrice}</p>}
								<p className="text-xs text-gray-600">Price per pack of 20 cigarettes</p>
							</div>
						)}
						...
						{parseInt(formData.vapesPerWeek) > 0 && (
							<div className="space-y-2">
								...
								<input
									...
									className={`input-field text-lg ${validationErrors.vapePrice ? "border-red-300" : ""}`}
									placeholder="15.00"
								/>
								{validationErrors.vapePrice && <p className="text-sm text-red-600">{validationErrors.vapePrice}</p>}
								<p className="text-xs text-gray-600">Price per vape device or pod refill</p>
							</div>
						)}
			...
			case 4:
				...
				const calculateEstimatedQuitDate = () => {
					...
					const rates = { slow: 0.05, medium: 0.1, quick: 0.15 };
					...
					while (remaining > 1 && weeksToQuit < 100) {
						...
						for (let i = 0; i < periodsThisWeek && remaining > 1; i++) {
							if (formData.reductionFrequency === "weekly") {
								remaining = remaining - totalConsumption * reductionRate;
							} else if (formData.reductionMethod === "compound") {
				...
				return (
					<div className="space-y-6">
						<div className="text-center">
							...
							<p className="text-gray-600 mt-2">Your personalized quitting plan is ready</p>
						</div>
						...
						<div className="bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50 p-6 rounded-xl border-2 border-green-300">
							<div className="text-center">
								...
								<p className="text-xs text-green-600 mt-3 italic">"Every journey begins with a single step. You've got this!"</p>
							</div>
						</div>
								...
								<>
									...
									<div className="flex justify-between items-center">
										...
										<span className="font-bold text-gray-900">
											...
											{parseFloat(formData.cigarettePrice).toFixed(2)}
										</span>
									</div>
								</>
							)}
							{parseInt(formData.vapesPerWeek) > 0 && (
								<>
									...
									<div className="flex justify-between items-center">
										...
										<span className="font-bold text-gray-900">
											{formData.currency === "USD" ? "$" : "€"}
											{parseFloat(formData.vapePrice).toFixed(2)}
										</span>
									</div>
								</>
							)}
						...
						<div className="bg-blue-50 p-4 rounded-lg">
							<p className="text-sm text-blue-800">
								📱 <strong>Important:</strong> We'll ask for notification permission on the next screen so we can remind you when
				...
				<div className="card">
					...
					<div className="mb-8">
						<div className="flex justify-center mb-2">
							<span className="text-sm font-semibold text-gray-600">Step {step + 1} of 5</span>
						</div>
						<div className="flex gap-2">
							{[0, 1, 2, 3, 4].map((i) => (
								<div
									key={i}
									className={`h-2 flex-1 rounded-full transition-all ${i <= step ? "bg-primary-500" : "bg-gray-200"}`}
								/>
							))}
						</div>
					</div>
