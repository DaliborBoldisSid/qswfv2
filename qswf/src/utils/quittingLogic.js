...
const REDUCTION_RATES = {
	...
	[PLAN_SPEEDS.MEDIUM]: 0.1, // 10% reduction per week
	...
	while (currentTotal > 0.5) {
		...
		if (reductionFrequency === "daily") {
			if (reductionMethod === "linear") {
				// Linear: subtract the baseline percentage evenly across the week to avoid compounding
				currentTotal = Math.max(0, currentTotal - dailyReductionAmount * 7);
			} else {
				// Compound: Multiply by the per-day rate while respecting doze constraints
			...
			// Weekly reduction is now linear against the original baseline so each week removes the same amount
...
export const canSmokeNow = (type, lastLog, currentWeekPlan, quitPlan = null, logs = []) => {
	...
	const waitTime = type === "cigarette" ? currentWeekPlan.waitTimeCigs : currentWeekPlan.waitTimeVapes;
