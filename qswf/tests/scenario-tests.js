...
function generateQuitPlan(userData) {
	...
	const hasBaseline = totalPerWeek > 0;
	const cigPercentage = hasBaseline ? cigarettesPerWeek / totalPerWeek : 0;
	const vapePercentage = hasBaseline ? vapesPerWeek / totalPerWeek : 0;
	...
	const weeklyReductionAmount = totalPerWeek * reductionRate;
	const dailyReductionAmount = totalPerWeek * (reductionRate / 7);
	...
	while (currentTotal > 0.5) {
		...
		if (reductionFrequency === "daily") {
			if (reductionMethod === "linear") {
				currentTotal = Math.max(0, currentTotal - dailyReductionAmount * 7);
			} else {
			...
			const nextTotal = currentTotal - weeklyReductionAmount;
			currentTotal = Math.max(0, nextTotal);
		}
...
function runScenarioTests() {
	...
	SCENARIOS.forEach((scenario, index) => {
		...
		console.log(
			`        ${scenario.config.planSpeed} pace, ${scenario.config.reductionMethod} ${scenario.config.reductionFrequency}`
		);
		...
		try {
			...
			if (plan.totalWeeks < scenario.expectations.minWeeks || plan.totalWeeks > scenario.expectations.maxWeeks) {
				errors.push(
					`Duration: expected ${scenario.expectations.minWeeks}-${scenario.expectations.maxWeeks} weeks, got ${plan.totalWeeks}`
				);
			}
			...
			for (let i = 0; i < plan.weeks.length; i++) {
				...
				if (sum !== week.totalAllowed && week.totalAllowed !== 0) {
					errors.push(
						`Week ${i}: Sum mismatch (${week.cigarettesAllowed} + ${week.vapesAllowed} = ${sum}, total = ${week.totalAllowed})`
					);
			...
			if (errors.length === 0) {
				...
				console.log(
					`   Week ${Math.floor(plan.weeks.length / 2)}: ${
						plan.weeks[Math.floor(plan.weeks.length / 2)].cigarettesAllowed
					} cigs, ${plan.weeks[Math.floor(plan.weeks.length / 2)].vapesAllowed} vapes`
				);
				passed++;
			} else {
				console.log(`\n❌ FAIL`);
